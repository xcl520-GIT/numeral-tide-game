...
## 工程亮点（若你是来评估能力的，先看这一节）

这个仓库里**最值得看的不是游戏本身，而是"改动怎么被验证"这一整套做法**。四样东西：

1. **[`工程方法.md`](工程方法.md)** —— 三道门禁、四条纪律、**9 条被数据否掉的方案**，
   以及**我和 AI 的分工**（我定判据与验收，AI 跑量与整理）+ **7 条挡 AI 幻觉的机制**
   （每条都配一次真实事故）。**这一份最能说明问题。**
2. **[`技术实现.md`](技术实现.md)** —— **代码层面的六处难点与取舍**，每节给落点（文件/函数）：
   零素材的参数化像素生成器、"随机但每层都真的走得通"的地图与确定性兜底、
   一群怪怎么"围上来"而不是排成一条线、A* 里"水不是墙"与把区域约束收进寻路层、
   一个饱和伤害公式与两种对付防御的手段、把"玩家"做成可复现的测量装置。
3. **[`开发记录/`](开发记录/README.md)** —— 31 份立项 / 取证 / 验收 / **证伪**记录，
   每份都能单独看懂，且**每条结论都写了怎么复现**。
4. **`数值潮汐/tools/`** —— 工具链：300 项集成冒烟、与游戏**共用同一份 `core.js`** 的
   900 局无头平衡模拟、语法自检、一键构建，以及 26 支一次性取证探针。
5. **`数值潮汐/src/js/core.js`** —— 单文件内核 **5424 行**（纯逻辑、不碰 DOM）：
   地图生成 / 区域 / 视野 / A* 寻路 / 战斗 / 掉落 / 潮汐 / 无头 AI。
   （全套源码 11 个文件、**13953 行 / 770 KB**。）

**几个数字**：零第三方运行时依赖 · 零素材文件（地形、角色、怪物全部由代码实时绘制）·
发行包 **677 KB** · 集成冒烟 **300 项** · 平衡门禁 **900 局 0 卡死** ·
每一处难度改动都附 **n=900 前后对照 + 一行回退方式**。

**它是什么**：一个把「**让玩家自己算**」当核心卖点的 roguelike —— 面板只给入参
（攻 / 穿 / 防），不给结论；每一场遭遇都要你自己判断"打不打、值不值"。

**技术栈**：原生 JavaScript（无框架、无构建步骤、无 npm）+ Canvas 2D
（地形 / 角色 / 怪物全部由代码实时绘制）+ Web Audio（音效与 BGM 全部合成，无音频文件）
+ C# 启动器（WebView2 原生窗口）。**零第三方运行时依赖、零素材文件、可完全离线。**

**开源**：MIT 许可证。想改代码从 `数值潮汐/src/` 开始，改完跑一遍冒烟测试与平衡模拟
（见「十三、开发与验证」）；只想玩就下 `数值潮汐/release/数值潮汐-v11.11.zip`。
...
1. 下载 `数值潮汐/release/数值潮汐-v11.11.zip`（约 **677 KB**）
2. 解压到任意目录，双击 `数值潮汐.exe`（解压后约 2 MB，免安装、可拷 U 盘、可传网盘）

不需要装任何运行库：渲染用的是 Windows 10/11 自带的 WebView2 运行时。
...
源码就是 `数值潮汐/src/` 里那几个文件，改完刷新即可 —— 没有编译、没有打包、没有依赖安装。
（⚠ 用 `file://` 直接打开 `src/index.html` 会因为浏览器安全策略读不到 `js/`，所以要用一个本地 http 服务。）

### 三、想验证数值 / 改平衡

三层门禁都在 `数值潮汐/tools/` 里，用系统自带的 Edge 无头模式跑，**不需要 Node**：

| 验什么 | 入口 | 当前结果 |
| :--- | :--- | :--- |
| 集成行为（真实点击 / 拖拽 / 渲染 / 出错兜底） | `tools\smoke.html` | **300 项 ALL PASS** |
| 平衡（和游戏共用同一份 `core.js`，跑 900 局） | `tools\sim.html?n=900&diff=all&cls=0` | 71.2 / 42.6 / 22.0 |
| 语法与模块加载 | `tools\loadcheck.html` | 12 / 0 |

参数、环境坑、以及"为什么这些测试要这么写"，见「十三、开发与验证」。
...
└─ 开发记录/                     研发日志（本项目最花时间的部分，全部公开）
   ├─ 设计迭代记录.md              完整迭代过程与踩过的坑
   ├─ v11.x-*.md                  每一轮的立项 / 取证 / 验收 / 证伪
   └─ 平衡基线-*.md                各阶段的三档实测数据（含怎么复现）
```
...
它把真实页面塞进 iframe，检查 **300 项**：模块加载、图标引用完整性、
...
1. **给热对象加字段 = 整局慢 3 倍。** `_makeEnemy()` 这类每回合都要读的对象，
   字段必须在字面量里一次声明完；事后再挂新字段会触发隐藏类迁移。
2. **无头 `--dump-dom` 下 `requestAnimationFrame` 不跑**，所以渲染类探针要自己驱动帧。
3. **同一个文件，不同工具报的行号可能不同** —— 改代码请用**文本锚点**定位，别用行号。
4. **`--virtual-time-budget` 会让 `--dump-dom` / `--screenshot` 挂死**；截图命令不要加它。
5. **残留的无头浏览器实例会占着 profile**，下一次启动的输出就是 0 字节。
   `tools\run_page.ps1` 已经把整条处置顺序封在里面（先杀带 `--headless` 且匹配本项目
   profile 的实例 → 等输出文件 flush → 再读）。⚠ 两个 `run_page.ps1` **不能并行**。
6. **控制台是 GBK**：中文输出取数用 `-out` 落文件，再读文件。
7. **`-url` 少写一层 `tools/` 会 404**，而 404 页面照样有 300KB 输出、照样报 `bytes=` ——
   取数后要确认内容里不是错误页。
8. **探针取"平均值"而不是"最后一局"**：怪模板是随机抽的，单局读数不可比。
9. **同一个指标在不同探针里的口径必须一致**，而且**样本量要一起读**：
   `n=300` 与 `n=900` 的同一个指标可以差 3~4 个百分点。
10. **内容新增会重排随机流**：往 `ENEMIES` 表里加一条，抽签序列就变了，
    "改动前 / 改动后"根本不是同一批世界。所以**难度验收要用 `n=900`**，
    `n=300` 只够看方向 —— 这一条是 v11.11 用两轮白调的参换来的。
11. **断言里不要写死参数**：能从数据表现读的就现读。`deepCorrupt` 一调值，
    写死 `6%` 的断言就 FAIL，而它报出来的样子和"机制坏了"**一模一样**。
12. **测试桩要不对称**：如果一次判据里"前后两次采样"被同一个桩偏移，偏移会互相抵消，
    那条断言等于没测（v11.11 白测过一次）。
13. **别拿环境的巧合当断言**：例如"这张图上一定找得到某个特殊落点" ——
    地图是随机生成的，这种断言会时绿时红。要测就**主动构造**那个条件。
14. **中文路径下 `grepContent` 不可用**，用 `Select-String`。
15. 一次性探针放 `_probe/`（不入库，见 `.gitignore`）；放进 `tools/` 会跟着入库。

### 改这个项目时的三条红线

1. **`rawDamage()` 一行不改。** 它是"改动可量化"的地基：伤害 = `atk·50/(def+50)`。
   要给防御找一个对手，用的是**穿透**（在公式之前削 `def`），而不是动公式本身。
2. **纯结构改动的验收标准是"逐位相同"。** 不改变行为的重构，必须能证明它没改变行为。
3. **可选字段 = 天然开关。** 新机制一律写成"不写就是旧行为"：这样每一项都能单独回退，
   也能和上一版逐位对照。

### 写测试的三条纪律

- **不可复现的测试等于没有测试。** 所有 `start()` 都传固定 seed
  （第一版没传，于是"画面亮度"在两次运行间从 25.3 跳到 79.2 —— 那不是回归，是开局地图不一样）。
- **能逐位比对就逐位比对。** "看起来合理"不算证据。
- **每个结论都要有一份能重跑的原始输出**，落在 `_probe/` 里（本地留存、不入库），
  并在 `开发记录/` 里写清怎么复现。
...
| 干净分发包 | `release\数值潮汐\` 与 `release\数值潮汐-v11.11.zip` | 只含可运行文件，传网盘用 |
...
- **加一种敌人**：`src/js/data.js` 的 `ENEMIES` 表里加一条即可。
  `shape` 决定它长什么样（参数化像素图，不需要美术资源）；`range` 不写就是贴脸怪、
  写了就是远程怪；抗性标签由 `tagsOf()` 从数值自动推导，不用手写。
- **加一种区域类型**：`data.js` 的 `REGIONS` 加一项 + `core.js` 的 `_buildRegions()`
  池子里加上它的名字 + 给它写一条**规则**（`_applyRegionFlavor()`，或像 `_carvePillars()`
  那样单独一个方法）。注意区域划分跑在敌人布点**之前**，动地形必须在那之前做完。

改完请跑这两条，并把数字写进 `开发记录/`：

```
tools\smoke.html                        → 300 项 ALL PASS
tools\sim.html?n=900&diff=all&cls=0     → 0 stall / 0 timeout，三档偏移要能解释
```

### 反馈

玩下来任何感受都欢迎，尤其是**手感类**的：哪一档太难、哪件装备没用、哪只怪看不懂。
这个项目的难度平衡是被反馈推着走的 —— `数值潮汐测试反馈.txt` 里那 6 条，
每一条都能在 `开发记录/` 里找到对应的立项、取证与验收。

**两个渠道都行：**

- **GitHub Issues**：<https://github.com/xcl520-GIT/numeral-tide-game/issues>
- **微信 / QQ 直接发我**：连截图一起，按下面这个格式写最有用 ——
...
（这是真实收到过的一份反馈的格式：**分条、说现象、不用给结论**。
"感觉哪里不对"比"应该改成这样"有用得多。）

### 许可证

**MIT** —— 随便用、随便改、随便分发，署名即可（全文见 `LICENSE`）。

### 致谢

- **图标**：[game-icons.net](https://game-icons.net)（CC BY 3.0）——
  实际用到的图标已内联进 `src/js/icons.js`，原始 SVG 留档在 `数值潮汐/assets-raw/`。
- **设计参考**：《元气骑士》的魂技、武器融合、以及地牢房间的连接方式，
  拆解笔记见 `元气骑士游戏设计逻辑拆解.md`。本项目**没有使用它的任何素材或代码**。
- **美术**：地形、角色、怪物全部由代码实时绘制（`src/js/art.js`），没有一张外部图片。
- **开发方式**：作者负责设计、判据与验收；AI 作为**开发提效工具**参与
  （批量跑平衡模拟、写测试断言、整理研发日志）。所有设计决策与验收标准由作者负责。
