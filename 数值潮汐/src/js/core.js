  ...
  class Game {
    ...
    constructor(opts) {
      ...
      /* P1（v11.5）：让等待有代价。两个**互相独立**的开关 ——
         ...
         ★ v11.11：waitCost 转正，默认 **'b'（累积惊动）**。选 b 不选 a 的理由沿用
         v11.5 的对照：a 直接改写潮汐节拍表，b 只借用"已醒的敌人"这条现成路径
         （"它察觉得到你"的距离全游戏只该有一个）。

         ⚠ **它对模拟基线逐位无影响 —— 那是预期的，不是"改了没用"。**
         无头 AI 从不等待：v11.5 两次给它造等待动机都被数据否掉
         （盯姿态时等待占比 0.0~0.1%、盯魂技冷却时 0.2~1.0%），
         所以这条代价在模拟器里**永远不会被触发**。它是一条**只对真人生效**的规则，
         冲着反馈里那句"潮汐压迫感不强，可以很自在地刷回合"来的。
         验收因此不是"数字变了"，而是"数字**逐位不变**" ——
         后者证明这条代价没有偷偷碰到任何别的东西。 */
      this.waitCost = (opts.waitCost === 'a' || opts.waitCost === 'b') ? opts.waitCost : 'b';
      ...
      /* P4（v11.5）：Boss 行为差异化。**v11.11 起默认开**（转正）。
         为什么是"一个总开关 + 模板上一条 bossBehavior"两层：
         总开关负责**对照实验**（代价与收益只能靠 A/B 回答），
         模板字段负责**逐只回滚**（某个行为单独不合适时不必把三个一起关掉）。

         ★ 为什么现在敢从"默认关"转成"默认开"：
         当初压着它的是一个**异常终止** —— bossfx=1 跑 2700 局会出现 1 例
         stall / timeout，而 bossfx=0 是 0 例。v11.5 P4 判定"它与 P4 无关，
         是无头 AI 自己的盲点，属于下一轮单独做的一件事"，就搁下了。
         v11.11 把那件事做完了 —— 根因是 AI 的四处缺陷（**没有一处是 bossfx 的 bug**）：
           ① _walkToward 的 A* 步数上限只有 4000。实测那 268 次失败里
              **28.4% 只是"搜不到"**（抬到 20000 就能找到路），剩下的是真不连通。
           ② "站在目标格上"那个出口既不记账也不离开，于是原地空转 40 次 = stall。
              渊喉的吼是**直接改 px/py** 的，会把玩家搬到宝箱格上而不消费它 ——
              那正是 bossfx 提高撞见率的原因（所以它看起来"像 bossfx 的 bug"）。
           ③ 门锁着（= 本层 Boss 还活着）时还把 exit 当目标 —— 走上去也只能干站着。
           ④ _pickTarget 会给"打起来太亏"的目标减 90 分，于是 AI 一路绕开 Boss
              刷杂兵、永远开不了门（那一例现场的 kills=751 就是这么来的）。
         修完之后：bossfx=1 的 2700 局与 bossfx=0 一样，**三档 0 stall / 0 timeout**。
         实测代价（n=900）：休闲 0 / 标准 −0.2pp / 深渊 0 —— 与 v11.5 当初的结论一致，
         P4 改的是**行为与体验**，不是数值平衡。传 0 仍可回到旧行为（三个 Boss 只有数值差别）。 */
      this.bossFxOn = (opts.bossFx === undefined) ? true : !!opts.bossFx;
      /* P3 第二步（v11.5）：魂技可以在**对决里**放一次。
         默认 **1 = 开** —— 这是本轮要交付的玩法本身，不是可选的难度旋钮。
         传 0 得到的是"魂技只在地图上放"的旧行为，也就是对照组；
         "这个改动值多少"只能由它回答（本项目所有代价都必须能被 A/B 量出来）。

         注意它当初和 bossFxOn 的默认**相反**（那个默认关），而且理由是反的：
         Boss 行为改的是**难度**，必须先证明它在噪声带内才敢开；
         魂技改的是**玩家侧的新选项**，关掉它等于这一轮什么都没做。
         （v11.11 起两条都默认开了 —— bossFxOn 那个异常终止修掉之后，
           "它在噪声带内"这件事有了 2700 局的证据：−0.2pp 落在噪声里。） */
    ...
    _autoStep() {
      ...
      if (!g) {
        ...
        /* ★ v11.11：门只有在本层 Boss 死后才开（见 _enterTile 里那句
           "潮汐之门被封锁着 —— 先解决这层的守卫"）。
           所以**门还锁着时，把 exit 当目标是无效的**：走上去也只能干站着，
           而空转不推进回合 —— 连续 40 次就是 stall。
           这时该做的是把 Boss 解决掉：**即使本层已经刷够怪**
           （layerKills 到了 leaveLayerAfterKills 这个阈值），Boss 也非打不可，
           它是门的唯一钥匙。 */
        if (!g && this.bossAlive) {
          /* ★ v11.11：门锁着时，Boss 不是"一个可打可不打的敌人"，它是门的**唯一钥匙**。
             所以这里直接锁定它，不把选择权交给 _pickTarget ——
             那个评分函数会给"打起来太亏"的目标减 90 分（预计承伤 > 45% 血），
             于是 AI 会一直绕着 Boss 刷杂兵、永远开不了门。
             实测：标准档 2700 局里那 1 例 timeout 正是这个形状 ——
             kills=751（一路在打杂兵）、bossAlive=true、而门就在 3 格外。
             也刻意**不看黑名单**：Boss 是唯一钥匙，而地形会变
             （潮水涨退会改变水路代价），一次走不到就永久放弃它，
             等于把这一层直接判成死局。走不到时的兜底见函数末尾。 */
          const b = this.boss;
          if (b && this.enemies.indexOf(b) >= 0) {
            g = { type: 'enemy', ref: b, x: b.x, y: b.y };
          } else {
            const t = this._pickTarget();
            if (t) g = { type: 'enemy', ref: t, x: t.x, y: t.y };
          }
      ...
      if (g.type === 'enemy') {
        ...
        if (g.x === this.px && g.y === this.py) {
          ...
          /* ★ v11.11：站在目标格上又"没被消费"时，必须**离开这一格**。

             为什么会有这种状态：渊喉的吼把玩家推开时是**直接改 px/py**、
             不经过 _enterTile，所以落在宝箱 / 泉水格上时格子不会被消耗；
             而 _nearestTile 会返回"距离 0"的它 → 每轮重选 → 每轮走到这里，
             空转 40 次被判 stall（实测 2700 局 1 例；同样样本 bossfx=0 是 0 例）。

             ⚠ exit **绝不能拉黑**：它是唯一的出路，一旦进了黑名单，
             门开之后也没人再选它 —— 实测那样修会把 1 例 stall 换成 **12 例 timeout**
             （AI 站在门边一直等到 2400 回合上限，trace 里每格都在动、位置却不变）。
             门为什么锁着由上面那条 `bossAlive` 分支负责（先去把钥匙打出来）。

             商栈那一条**保持原样**（它在上面就 return 了）：它的空转一轮是设计
             （消费掉"专程跑商栈"这件事），不该被改成多走一步。 */
          if (g.type !== 'exit') this._badGoal(g);
          this.aiGoal = null;
          for (const d of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
            const nx = this.px + d[0], ny = this.py + d[1];
            if (!this.walkable(nx, ny)) continue;
            if (this.enemyAt(nx, ny)) continue;
            if (this.stepTo(nx, ny)) return true;
          }
          return false;   // 四面都被墙或敌人堵死才会到这里
        }
      ...
      this._badGoal(g);
      this.aiGoal = null;
      /* ★ v11.11：如果连"唯一的出路"都走不到，就**消耗一回合**而不是空转。
         · exit 是这一层唯一的出口；Boss 是门锁的唯一钥匙。
         · 空转不推进回合 —— 连续 40 次就会被 playHeadless 判成 stall，
           而 stall 会把整局从统计里作废，让真正的数值结论被一个空转掩盖掉。
         · 等待让潮水与敌人继续变化（水位涨退会改变水路代价），局面可能自己解开。
         实测：标准档 2700 局里那 1 例 timeout 就是这个形状（走不到 Boss）。
         这条只在这两种目标上生效，普通敌人走不到仍然照旧返回 false（拉黑换目标）。 */
      if (g.type === 'exit' || g.ref === this.boss) {
        this.wait();
        return true;
      }
      return false;
    ...
    _walkToward(tx, ty) {
      ...
      /* ★ v11.11：4000 → 20000。
         4000 是上一轮抬过的值，理由和这里一样；但实测它还不够：
         休闲档 900 局里 _walkToward 一共失败 268 次，**全部**都是"目标格自身可走"
         （即不是目标非法，而是搜不到），其中把上限抬到 20000 就有 **76 次（28.4%）**
         能找到路 —— 而再抬到 60000 一次都不增加，说明 20000 就是够的那个数。
         剩下那 71.6% 是**真的不连通**（距离只有 6~18 格却三种上限都搜不到），
         那是地图生成的事，不在这一层解决。
         代价：只有"本来就找不到路"的调用才会跑到上限，所以性能影响可忽略
         （失败率约 0.3 次/局）。 */
      const path = this.findPath(this.px, this.py, tx, ty, 20000);
      if (!path || path.length < 2) return false;
      const n = path[1];
