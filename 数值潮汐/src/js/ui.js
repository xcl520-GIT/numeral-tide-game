...
(function (global) {
  ...
  /* ============================================================
     ...
     但要诚实说清楚它值多少：v11.4-f（A1）之前实测 83% 的对决在第 1 轮就分胜负
     （普通怪 92%），那时对大多数战斗来说菜单只会弹一次 ——
     那一轮的选择和"开战前选一次"是同一个决定。
     A1 把野生普通怪拉到 2.4 / 2.4 / 2.0 轮、一轮结束 28% / 29% / 43% 之后，
     弹第二次已经成了常态 —— 而第二次才是它存在的理由：
     那一刻敌人可能已经翻了面，题目跟着变了。
     姿态每 2 次行动翻一面（v11.4-f 由 3 改 2），正是为了让这件事发生。
     ============================================================ */
