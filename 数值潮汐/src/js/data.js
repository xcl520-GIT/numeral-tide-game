...
(function (global) {
  ...
  /* ============================================================
     ...
     **可选字段 = 天然开关**（本项目一贯的规矩）：不写就是旧行为。
       · `range` —— 攻击距离。不写 = 1 = 贴着脸打；写了就是远程怪，
                    够得着就在原地出手，不再往上贴（执行点见 core.js `_reach`）。
       · `spawn` —— 会孵化（孢子母与骨潮督军共用同一条路径）。
       · `loot`  —— 保底掉落（宝箱怪）。
     ============================================================ */
  const ENEMIES = [
    ...
    /* —— 远程的两只（v11.11 · 反馈⑥「敌人种类少，可以增加法师、弓箭手」）——
       和前面四只的差别**只有一条**：`range`。它写的是"攻击距离"，
       不写就是 1 = 贴着身打。所以这两只是**行为**上的新品种，不是数值上的。

       它们的代价也写在这里，免得以后被当成白拿 —— 而且这个代价是**量出来**的，
       不是拍的。把这条调参轨迹完整留下，因为它同时记下了一条读数纪律：

       · 第一版 血量 72/62 · 每击 6/8 · 权重 6/5
         → n=300：休闲 63.3% / 标准 37.3% / 深渊 19.0%，第 1 层死亡
         （v11.8 定的难度温度计）休闲 0% / 标准 +9.4% / **深渊 +14.2%**。
       · 第二版 权重 4/3、每击 5/6 → 65.3 / 39.0 / 19.7，第 1 层死亡回到 ±10% 内。
       · 第三版 权重 3/2、每击 4/5 → 70.3 / 38.3 / 20.0。
       · 第四版（定稿）弓手射程 4 → 3、术士每击 5 → 4 → 72.3 / 38.0 / 19.3。

       ⚠⚠ 这条轨迹里最重要的一行不是数字，是**尺子本身**：
         **往 ENEMIES 表里加条目会重排整条随机流。** `_enemyPool` 是按权重
         累积和抽签的，池子里多两项，同一个随机数就落到另一只怪身上 ——
         于是"改动前 / 改动后"根本不是同一批世界，两边的差里混着一次完整的
         重新生成。证据就在上面：标准档从第二版到第四版，代价砍了两轮，
         差值却几乎不动（−3.7 → −4.4 → −4.7pp）；而同一份定稿换 **n=900**
         去量，只差 −1.1pp（72.1 / 40.9 / 19.6 对 71.8 / 42.0 / 19.7）。
         ⇒ 内容新增的难度验收，**n=900 才算尺子，n=300 只够看方向**。
           下一次别再拿 n=300 的 pp 去调代价 —— 那是在追一个样本的运气。

       顺带查到、也值得留给下一次的一条事实（是事实，不是结论）：
         砍权重会让池子**总权重**变小，tier2/3 精英的**占比反而升高**
         （28.6% → 31.6%），所以"少放远程怪"不是一条免费的路。

       形态上的取舍：两只的射程现在相同（都是 3 格），它们的区分改由
       **伤害类型**承担（物伤 / 法伤），那正好接上本作「物抗 / 法抗」那条教学线。
       防御一律给 3/3：既不进姿态（canStance 要求双侧 >= minDef），
       也不挂抗性标签 —— 它身上只有「远程」这一条线索。 */
    { id: 'archer', name: '弓手',   tier: 1, weight: 3, kind: 'normal', range: 3,
      shape: { body: 'robed', w: 19, h: 19, eye: 2, color: '#c8b46a', accent: '#7a5a1a', spikes: 1, arms: 2 },
      stats: { hp: 72, atkP: 4, atkM: 0, defP: 3, defM: 3, spd: 12 },
      note: '隔着 3 格放箭 —— 不走近就打不着它，而走过去的路上它一直在射' },

    { id: 'caster', name: '术士',   tier: 1, weight: 2, kind: 'normal', range: 3,
      shape: { body: 'robed', w: 18, h: 21, eye: 3, color: '#a88ce0', accent: '#4a3a78', spikes: 0, arms: 2 },
      stats: { hp: 62, atkP: 0, atkM: 4, defP: 3, defM: 3, spd: 9 },
      note: '隔着 3 格施法 —— 只用法术打人，所以「高法抗」那件装备一次挡两个' },
  ...
  const DIFFICULTIES = {
    ...
    abyss: {
      ...
      depth: 5, enemyScale: 0.250, enemyMul: 1.85, healAfterWin: 0.040,
      tideEvery: 8, healFountain: 0, tierBoost: 1, extraLife: 0,
      /* v11.10：回血 0.024 → 0.040（= 标准档）—— 「顶部过重」终于被**搬**走了，
         而不是降下去。

         为什么只有"补给"这一条路可走：第 1 层**不吃 enemyScale**
         （(1+s·(depth-1)) 在 depth=1 时为 1），所以它的强度旋钮只有三个 ——
         数量（enemyMul）、精英（tierBoost）、补给。前两个 v11.8 已干净证伪：
         怪数砍半 → 第 1 层死亡 200→206（不动）而通关率塌到 15%；
         关掉精英提前 → 210（更差）。因为**敌人既是威胁也是资源**
         （每 4 杀升级、每 6 杀秘藏）。
         ⇒ 补给是第 1 层唯一还剩的杠杆。这也解释了 v11.8 那张表为什么
           "每一条有效的药方都在降难度"。

         转折点在 v11.9：后半程装上可调加压阀（deepCorrupt）之后，
         "削前两层 + 补后半程"第一次可以**不是降难度**，而是**搬家**。
         实测（n=900，seedBase=1000，fumble=0.2，见 _probe\v11.10\）：
           通关率      21.8% → 22.8%（几乎不动）
           总死亡      704 → 695（**守恒** —— 这是"搬走"与"降下去"的分水岭）
           第 1 层死亡  627 → 541
           到达第 3 层  241 → 305（+27%，更多局看得到后半程）
           第 3-4 层死  40 → 94
           条件概率     16.6% → 30.8%
           stalls     无
         代价已记账：单局回合 199.9 → 231.7（**+16%**）、深渊通关率 +1.0pp。
         ⚠ 这一行和下面的 deepCorrupt: 0.105 是**一组**，一起看才对；
           单独改一个都是难度变更而不是搬家。
         回退：把这里改回 0.024、把 deepCorrupt 改回 0.06，无代码改动。 */
      /* P5（v11.7）：速度碾压的阈值 —— 深渊比全局（18）更宽。
         ...
         **删掉这一行即回退**（回到全局 18）。 */
      speedGap: 26,
      /* v11.9：**深层的潮水**。第 3 层起，每场战斗结束后扣最大生命的这个比例。
         来历：v11.8 量出深渊的难度曲线"前重后空"（{1:200, 2:11, 3:4, 4:1, 5:84}
         —— 死在第 3-4 层的只有 5 局），而 v11.9 第二发找到了原因：
         **怪的伤害被"防御饱和 + 玩家血池暴涨"吃掉了**（第 5 层每场只掉 0.7 血，
         占玩家血 0.1%）。抬攻击在数学上无解（要 ×10，第 3 层会先爆），
         所以改用**无视防御、按最大生命比例**的真实伤害 —— 它与血池同比例，
         不会被稀释。0.06 是实测值：条件概率（到过第 3 层的局里有多少死在
         第 3-4 层）从 **5.6% → 20.2%**，通关率 28% → 23.3%，**第 1-2 层逐位不变**。
         删掉这一行即回退（= 旧行为）。
         ⚠ v11.10 把它抬到 **0.105** —— 它不是一次单独的难度上调，而是和上面的
         回血 0.040 配成一组「搬家」：回血让更多人活出前两层（分母变大、
         前两层死亡下降），腐蚀把这份死亡搬去第 3-4 层。单看这一行会误读。
         详见上面 v11.10 那段注释；探针在 `_probe\v11.10\probe_gate*.html`。 */
      deepCorrupt: 0.105,
      /* v11.10：**敌人的穿透**（每下一层 +8）—— 给防御一个对手。
         来历：`_scaleEnemyStats` 里 penP / penM 一直走"原值透传"，而 ENEMIES
         模板里没有一个写 penP，于是被兜底补成 0 —— **怪物的穿透恒为 0**。
         玩家的防御（均值 45 / 峰值 192）在打一场没有对手的仗。
         为什么不用"抬攻击"那条路：伤害 = atk·K/(def+K) 是饱和的，抬 atk 会先
         把低防玩家打死，对高防几乎无感（v11.9 已量化）。穿透是**减法**，
         削的是防御的绝对量 —— 它是唯一能"对抗防御"的手段；
         而 deepCorrupt 是"绕过防御"（无视防御的真实伤害）。
         实测（n=900，_probe\v11.10\probe_def5.html）：
           通关率 22.8% → 20.1%（−2.7pp）
           第 1 层死亡 **541 局逐位不变**（穿透从第 2 层起生效）
           装备防御的价值 6.9pp → **4.2pp**（仍然有用，但不再是万能）
         **删掉这一行即回退**（不写 = 0 = 旧行为；休闲/标准正是这样）。 */
      penPerDepth: 8
    }
  };
  ...
  const MAP = {
    ...
    rocks: 0.03,
    /* v11.11 · E（柱厅）：区域内埋石柱的步长。
       **2 = 只埋"横纵坐标都是偶数"的那种格** ⇒ 面积的四分之一，
       而且天生留着网格状的空档（任何一格都还能绕开柱子走）——
       所以下面的连通性复验正常情况下一次都不会响。这条性质是刻意的：
       兜底一旦响了，地图上就多一条笔直的应急走廊，那是"修好了"，
       但玩家看到的是一条不该出现的路。
       调成 3 就是九分之一（更稀，几乎看不出"这一区不一样"）；调成 1 会埋死整区。 */
    pillarStep: 2
  };
  ...
  /* ============================================================
     ...
     ★ 设计取舍：属性型标签（高物抗 / 高法抗 / 法术 / 汲血）由 core.js 的
       ...
       行为型标签（召唤 / 远程 / 精英 / 首领）推导不出来，因为它是行为不是数值，
  ...
  const REGIONS = {
    ...
    /* v11.11 · E：**地形型**区域（柱厅）。
       加它回应的是反馈第 1 条：「每层的变化感觉不大……其余地图感觉大差不差」

       为什么要单开一个"轴"：已有的四种其实只分两类 ——
         · 难度型（elite / hazard）改的是**敌人强度** —— 改的是"这一区有多难"；
         · 休整型（reward）改的是**敌人数量** —— 改的是"这一区安不安全"；
         · 而「每层看起来不一样」这件事，它们一条都没解决：
           怪强一点、怪少一点，地图**长得还是同一个样子**。
       柱厅改的是**地形的形状**，于是它同时给三样东西：
         ① 一眼：进区就看得出"这一区不一样"；
         ② 掩体：埋下去的石柱**同时挡视线**（OPAQUE）—— 而 v11.11 刚做的
            远程怪第一次让"视线"变成一条真的战术资源（看不见就不许开火）；
         ③ 卡位：窄口变多，"被围住"和"甩开它"的代价同时上升。

       danger 给 **1**：它是地形型，不是难度型。把它标成高危险会让 HUD 上
       那排星开始撒谎（"三颗星的区域其实没那么难"），而标签撒谎比没有标签更糟 ——
       这条规矩在敌人标签那边已经写过一次。enemMul 1.00 同理：这一区不额外加难。 */
    pillar: { key: 'pillar', name: '柱厅', color: '#8a8f98', tint: 'rgba(138,143,152,0.14)', danger: 1, enemMul: 1.00 }
  };
